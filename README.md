...
Build backend (docker compose build) then run (docker compose up -d), build frontend (npm install) then start frontend (npm run tauri dev) from the frontend directory
